...
$$p(y^{\ast} | x^{\ast}, \mathcal{D}) = \int p(y^{\ast} | x^{\ast}, \omega) \, p(\omega | \mathcal{D}) \, d\omega$$
...
$$p(y^{\ast} | x^{\ast}) \approx \frac{1}{T} \sum_{t=1}^{T} p(y^{\ast} | x^{\ast}, \omega_t), \quad \omega_t \sim q(\omega)$$
