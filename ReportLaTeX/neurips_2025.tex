...
\begin{itemize}
  ...
  \item \textbf{Brier score}:
        Combina en una \'unica cifra calibraci\'on y resoluci\'on~\citep{gneiting2007strictly}.
        \begin{equation}
...
La Tabla \ref{tab:timing_benchmark} resume un \emph{benchmark} controlado de inferencia sobre 20
batches de test previamente cargados en GPU, con el fin de excluir el
coste del \texttt{DataLoader} y medir \'{u}nicamente el c\'omputo del
modelo. En este escenario, el modelo determinista es el m\'as r\'apido
(0,179\,s para 1280 im\'agenes), mientras que la ruta probabil\'istica
media de Laplace basada en GLM-probit mantiene una latencia muy cercana
(0,201\,s). Cuando se incluye tambi\'en la estimaci\'on de incertidumbre,
Laplace alcanza 0,448\,s, aproximadamente \(2{,}5\times\) el coste del
baseline, pero sigue siendo muy inferior a MC Dropout, que requiere
8,831\,s debido a sus 50 pasadas estoc\'asticas. En conjunto, estos
resultados indican que la aproximaci\'on de Laplace ofrece un compromiso
favorable entre coste y utilidad probabil\'istica: la probabilidad media
puede obtenerse con una sobrecarga m\'inima respecto al modelo
determinista, y la inferencia completa con incertidumbre sigue siendo mucho
m\'as eficiente que MC Dropout.
...
\begin{table}[t]
\centering
\caption{Benchmark controlado de inferencia sobre 20 batches de test
(1280 im\'agenes) previamente cargados en GPU. Se reporta la media de 10
repeticiones tras 3 iteraciones de \emph{warm-up}. ``Laplace (media)''
corresponde a \texttt{predict\_proba} con regla GLM-probit, mientras que
``Laplace (completo)'' corresponde a
\texttt{predict\_with\_uncertainty}.}
\label{tab:timing_benchmark}
\begin{tabular}{lccc}
\toprule
Modelo & Tiempo total (s) & ms/imagen & Relativo al MAP \\
\midrule
MAP & 0,179 & 0,140 & 1,00$\times$ \\
Laplace (media) & 0,201 & 0,157 & 1,12$\times$ \\
Laplace (completo) & 0,448 & 0,350 & 2,50$\times$ \\
MC Dropout & 8,831 & 6,899 & 49,29$\times$ \\
\bottomrule
\end{tabular}
\end{table}
