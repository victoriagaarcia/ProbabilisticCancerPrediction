...
la Secci\'on~\ref{sec:triage}. En t\'erminos de discriminaci\'on, los tres modelos
presentan un empate pr\'actico: AUC-ROC \(\approx 0{,}9923\),
F1 \(\approx 0{,}95\) y \emph{accuracy} \(\approx 0{,}96\), con
diferencias relegadas a la cuarta cifra decimal. Este resultado indica
que la incorporaci\'on de cuantificaci\'on de incertidumbre no degrada el
poder discriminativo del clasificador base.

Las diferencias aparecen en calibraci\'on. La Figura~\ref{fig:reliability} muestra que los
tres modelos est\'an ya muy por debajo del objetivo no funcional
...
probabilidades mejor calibradas, especialmente en el caso de Laplace.

\begin{figure}[t]
  \centering
  % >>> AQU\'I VA: reliability_diagrams.png <<<
  \includegraphics[width=0.8\linewidth]{../figures/reliability_diagrams.png}
...
la Secci\'on~\ref{sec:map-to-bayes}: el Hessiano GGN captura la
...
cl\'inica directa: una incertidumbre aleatoria alta indica
...
de 96,31\% (MAP sin triaje) a 99,13\% (Laplace) y 99,04\% (MC~Dropout)
—véase la Tabla~\ref{tab:main}. Más relevante clínicamente, la tasa de falsos negativos
cae de \(\sim\!4{,}2\%\) a \(\sim\!1{,}1\%\) con Laplace y
\(\sim\!1{,}0\%\) con MC~Dropout. En términos absolutos, la política
bayesiana permite identificar 7–8 de cada 10 falsos negativos que emite el
...
\emph{coverage}) para ambos métodos bayesianos. En los dos casos la curva
es monótona creciente al reducir la cobertura —condición necesaria para
que una política de \emph{selective classification} tenga sentido—, y
ambas curvas son prácticamente indistinguibles entre sí: reteniendo el
\(\sim\!80\%\) de las muestras más confiables se alcanza \(\sim\!99{,}5\%\)
de exactitud. La elección del percentil de corte se convierte así en un
...
La inspección visual del \emph{top}-12 de muestras con mayor incertidumbre
epistémica (Ap\'endice~\ref{app:high-unc}) confirma que la señal se concentra en casos
genuinamente ambiguos: zonas con mezcla de tejido tumoral y estroma,
artefactos de digitalización o parches próximos al borde de la región
central etiquetada. Las probabilidades predichas en estos casos se sitúan
...
interpretación clínica del triaje: los ejemplos que el modelo considera
más inciertos son precisamente aquellos que más se benefician de revisión
humana.
