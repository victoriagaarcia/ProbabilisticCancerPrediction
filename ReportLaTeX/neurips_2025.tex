...
\begin{itemize}
  ...
  \item \textbf{Expected Calibration Error} (ECE): para cada muestra
        sea $\hat y$ la clase predicha y $\hat c=p(\hat y\mid\mathbf{x})$
        la confianza asociada. Agrupando las predicciones en $M=15$
        bins $\{B_{m}\}_{m=1}^{M}$ de ancho uniforme sobre la confianza,
        \begin{equation}
          \mathrm{ECE}
          \;=\;\sum_{m=1}^{M}\frac{|B_{m}|}{N_{\mathrm{test}}}
          \Bigl|\,\mathrm{acc}(B_{m})-\mathrm{conf}(B_{m})\,\Bigr|,
          \label{eq:ece}
        \end{equation}
        donde
        $\mathrm{acc}(B_{m})=\tfrac{1}{|B_{m}|}\!\sum_{i\in B_{m}}\mathbf{1}[\hat y_{i}=y_{i}]$
        y
        $\mathrm{conf}(B_{m})=\tfrac{1}{|B_{m}|}\!\sum_{i\in B_{m}}\hat c_{i}$.
